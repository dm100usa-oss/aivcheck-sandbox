...
function checkXRobots(headers: Headers) {
  const v = headers.get("x-robots-tag") || "";
  const blocked = /\bnoindex\b|\bnone\b/i.test(v);
  return item(
    "x_robots_tag",
    !blocked,
    v ? `X-Robots-Tag: ${v}` : "No X-Robots-Tag header (ok)"
  );
}
...
function checkMetaRobots(html: string | null) {
  ...
  return item(
    "meta_robots",
    !blocked,
    content ? `meta robots: ${content}` : "No meta robots (ok)"
  );
}

function checkCanonical(html: string | null, origin: string) {
  const href =
    textMatch(
      html,
  ...
  return item(
    "canonical",
    ok && sameOrigin,
    ok ? `canonical: ${href}` : "No canonical link"
  );
}

function checkTitle(html: string | null) {
  const t = stripTags(textMatch(html, /<title[^>]*>([\s\S]*?)<\/title>/i) || "");
  return item(
    "title_tag",
    t.length > 0,
    t.length ? `Title: ${truncate(t, 120)}` : "Missing <title>"
  );
}

function checkMetaDescription(html: string | null) {
  const d =
    textMatch(
      html,
...
function checkOpenGraph(html: string | null) {
  const t =
    textMatch(
      html,
  ...
  return item(
    "open_graph",
    ok,
    ok ? "og:title & og:description found" : "Missing Open Graph tags"
  );
}
...
function checkViewport(html: string | null) {
  const v =
    textMatch(
      html,
  ...
  return item(
    "mobile_friendly",
    ok,
    ok ? `viewport: ${v}` : "Missing/invalid viewport meta"
  );
}

function checkAltAttributes(html: string | null) {
  ...
  return item(
    "alt_attributes",
    ok,
    `Images with alt: ${withAlt}/${total} (${Math.round(ratio * 100)}%)`
  );
}
...
async function check404(origin: string) {
