...
export interface AnalyzeResult {
  ...
  mode: Mode;
  items: CheckItem[];
  score: number; // weighted over all 15 checks
...
export async function analyze(rawUrl: string, mode: Mode): Promise<AnalyzeResult> {
  ...
  const all: Record<CheckKey, CheckItem> = {
    ...
    https: {
      ...
      name: nameOf("https"),
      passed: schemeOk,
      description: schemeOk ? "HTTPS detected" : "Page not served via HTTPS",
    },
...
function calcWeightedScore(all: Record<CheckKey, CheckItem>): number {
  const total = PRO_KEYS.reduce((a, k) => a + weightOf(k), 0); // should equal 100
...
function checkJSONLD(html: string | null): CheckItem {
  if (!html)
    return item("structured_data", false, "No JSON-LD structured data");
  const re =
